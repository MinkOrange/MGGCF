...
class AGREE(nn.Module):
    def __init__(self, num_users, num_items, num_groups, embedding_dim, group_member_dict, drop_ratio):
        ...
        # initial model
        for m in self.modules():#对上面的神经层参数初始化
    ...
    def grp_forward(self, group_inputs, item_inputs):
        ...
        item_embeds_full = self.itemembeds(Variable(torch.LongTensor(item_inputs)).cuda())
        for i, j in zip(group_inputs, item_inputs):
            members = self.group_member_dict[int(i)]
            members_embeds = self.userembeds(Variable(torch.LongTensor(members)).cuda())
            items_numb = []
            for _ in members:
                items_numb.append(j)
            item_embeds = self.itemembeds(Variable(torch.LongTensor(items_numb)).cuda())
            group_item_embeds = torch.cat((members_embeds, item_embeds), dim=1)#[members_embeds,item_embeds]
            at_wt = self.attention(group_item_embeds).cuda()#
            g_embeds_with_attention = torch.matmul(at_wt, members_embeds)
            group_embeds_pure = self.groupembeds(Variable(torch.LongTensor([i])).cuda())
        ...
        y = F.sigmoid(self.predictlayer(new_embeds).cuda())
        return y

    # user forward
    def usr_forward(self, user_inputs, item_inputs):
        user_inputs_var, item_inputs_var = Variable(user_inputs).cuda(), Variable(item_inputs).cuda()
